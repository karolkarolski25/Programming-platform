...
public class ChartAdvancedScatter extends Application {
    ...
    protected ScatterChart<Number, Number> createChart() {
        ...
        series1.setName("f(x) = -x^2");
        series2.setName("f(x) = x^2 - x + 3");

        for (int i =- 500; i <= 500; i++) 
        {
            series.getData().add(new XYChart.Data<Number, Number>(i, 0));
            series1.getData().add(new XYChart.Data<Number, Number>(i, -Math.pow(i, 2)));
            series2.getData().add(new XYChart.Data<Number, Number>(i, Math.pow(i, 2) - i + 3));
        }

        sc.getData().addAll(series, series1, series2);
